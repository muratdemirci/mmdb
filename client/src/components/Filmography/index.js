...
const Filmography = ({ data }) => {
  ...
  const Cell = (props) => {
    return <li className='m3-text-white'><span className='m3-text-orange' title={props.title} style={{ cursor: 'pointer' }} onClick={() => handleClick(props)}>{props.title}</span> as <span className='m3-text-khaki'>{props.character}</span></li>
  }

  const Col = (props) => {
    const movies = props.data
    const movieList = movies.map((movie) =>
      <Cell id={movie.id} title={movie.original_title} character={movie.character} key={movie.id} />
    )
    return (
      <div>
        <ul>
  ...
  return (
    <div>
      ...
      {Object.keys(data.chunk).reverse().map(function (date) {
        return (
          <div key={date}>
            <h3 className='m3-text-teal' onClick={() => toggle(date)}> {`👉 ${date || 'Unreleased'}`}</h3>
