...
export const getPersonFilmography = async (nameId) => {
  try {
    ...
    const rdSorted = [...response.data.cast, ...response.data.crew].sort(
      (a, b) => {
    ...
    const result = rdSorted.reduce(function (r, a) {
      if (
        a.release_date === undefined ||
        !a.character ||
        a.character.length === 0
      ) {
        return r
      }
...
export const getPersonKnownFor = async (nameId, limit) => {
  try {
    ...
    const knownFor = [...response.data.cast, ...response.data.crew].sort(
      (a, b) => {
